<template>
  <div v-if="show" class="fixed inset-0 bg-black/30 z-50 flex justify-center items-center" @click.self="!saving && $emit('close')">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-8 transform transition-all duration-300 ease-out" :class="show ? 'scale-100 opacity-100' : 'scale-95 opacity-0'">
      <div class="flex justify-between items-center mb-6">
        ...
        <button
          ...
          class="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-colors disabled:opacity-40"
          :disabled="saving"
        >
      ...
      <div class="mt-8 flex justify-end gap-4">
        <button
          @click="!saving && $emit('close')"
          class="px-6 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          :disabled="saving"
        >
          取消
        </button>
